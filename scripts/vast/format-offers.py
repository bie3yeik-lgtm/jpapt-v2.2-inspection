...
def normalize(pricing_type: str, source: Path, offer: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reliability": offer.get("reliability"), "dlperf": offer.get("dlperf"),
...
def render_markdown(inventory: dict[str, Any]) -> str:
    lines = [
        ...
        f"- search_query: `{inventory['search_query']}`",
        f"- offer_count: `{len(inventory['offers'])}`", "",
        "| Offer ID | GPU | VRAM GB | GPUs | Reliability | DLPerf | Driver | Direct ports | $/h total | $/h base | $/h storage | Min bid | Location |",
        "|---:|---|---:|---:|---:|---:|---|---:|---:|---:|---:|---:|---|",
    ]
    for offer in inventory["offers"]:
        lines.append(
            "| {offer_id} | {gpu_name} | {gpu_ram_gb} | {num_gpus} | {reliability} | "
            "{dlperf} | {driver_version} | {direct_port_count} | {dph_total} | {dph_base} | "
...
def main() -> None:
    ...
    for source in sorted(args.input_dir.glob("*.json")):
        if source.name in {"inventory.json", "search-query.json"}:
            continue
        payload = json.loads(source.read_text(encoding="utf-8"))
        for raw_offer in _payload_offers(payload):
            offers.append(normalize(args.pricing_type, source, raw_offer))
    ...
    with args.output_csv.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=("pricing_type", *FIELDS))
        writer.writeheader()
