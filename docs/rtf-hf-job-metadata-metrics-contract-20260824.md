...
6. RunPod billing historyに対象Podの明細がない場合はfail-closedとする。RunPod公式ドキュメントでは課金処理が5分周期であるため、`enrich_runpod_job_metrics.py`は eventual consistency を前提にリトライする。RTX 2000 Ada full-matrix smoke（Actions run 32871433137）では batch 1→8→32 の wall clock が約46分、batch 32 Pod 単体が約31分だった。既定はこの full-matrix 時間 + 5分課金周期 + 20分余裕 = 約71分（284回×15秒）で、`RTF_RUNPOD_BILLING_MAX_WAIT_SECONDS`、`RTF_RUNPOD_BILLING_ATTEMPTS`、`RTF_RUNPOD_BILLING_RETRY_SECONDS`で上書きできる。
