"""
JSON Schema validation for evaluation artifacts.
...
The schema files are the structural contract.

This module intentionally does not implement acceptance thresholds such as:

- allowed CER
- allowed WER
- allowed max_abs_error
- allowed relative_l2

Those belong to the locked HF Bucket:
...
class EvaluationSchemaError(RuntimeError):
    """
    Raised when an evaluation JSON artifact violates its JSON Schema.
    """
...
def _json_path(parts: list[Any]) -> str:
    """
    Convert jsonschema path components into a readable JSON path.
    """
...
def _normalize_instance(
    ...
    """
    Convert supported Python model values into JSON-compatible structures.
    """
...
class EvaluationSchemaRegistry:
    """
    Load and cache project evaluation JSON Schemas.
    """
    ...
    def load_schema(
        ...
        try:
            ...
            raise EvaluationSchemaError(
                f"Invalid JSON Schema file: {exc}",
                schema_name=schema_name,
            ) from exc
    ...
    def validate(
        ...
        instance_path = _json_path(
            list(first.absolute_path)
        )

        schema_path = _json_path(
            list(first.absolute_schema_path)
        )
        ...
        if len(errors) > 1:
            message += (
                f" ({len(errors)} validation errors total; "
                "showing the first)"
            )
...
def validate_sample_result(
    ...
    *,
    ...
    EvaluationSchemaRegistry(
        repository_root
    ).validate_sample_result(instance)
