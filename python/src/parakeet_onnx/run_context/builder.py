...
def _run_git(repository_root: Path, *args: str, allow_empty: bool = False) -> str:
    try:
        ...
        raise ContractError(f"unable to resolve git identity with {' '.join(args)!r}") from exc
...
def _git_identity(repository_root: Path) -> GitIdentity:
    ...
    ref = os.environ.get("GITHUB_REF") or _run_git(repository_root, "symbolic-ref", "--short", "-q", "HEAD")
...
def _runtime_identity(
    ...
    *,
    ...
    if not version:
        raise ContractError("runtime backend version is required; refusing an unknown execution identity")
    ...
    def build(
        ...
        *,
        ...
        if candidate.profile_set_id != self.revisions.runtime.profile_set_id:
            raise ContractError("candidate profile_set does not match revision runtime profile_set")
        primary = candidate.primary_artifact
        generated_candidate = candidate.generated_contract()
        created = datetime.now(UTC)
        extra = dict(metadata or {})
        if "candidate" in extra:
            raise ContractError("metadata.candidate is generated by the builder and cannot be overridden")
