...
use asr_capsule::{
    CapsuleManifest, CapsuleMetric, CapsuleSample, ExperimentCapsule, write_capsule,
};
use serde_json::Value;

use crate::{EvalError, Result, dataset::ResolvedManifest, writer::write_json};

pub fn write_evaluation_capsule(
    output: &Path,
    run_context: &Value,
    ...
    if samples.len() != input.samples.len() {
        return Err(invalid("capsule sample count does not match normalized evaluation input"));
    }
    ...
    push_metric(
        &mut metrics,
        benchmark,
        "/performance/session_creation_ms",
        "session_creation_ms",
        Some("ms"),
    )?;
    push_metric(
        &mut metrics,
        benchmark,
        "/performance/total_processing_ms",
        "total_processing_ms",
        Some("ms"),
    )?;

    let capsule = ExperimentCapsule { manifest, samples, metrics };
    let receipt = write_capsule(output.join("run.parquet"), &capsule)
        .map_err(|error| invalid(format!("failed to write evaluation capsule: {error}")))?;
    write_json(&output.join("capsule-receipt.json"), &serde_json::to_value(receipt)?)?;
    Ok(())
}

fn sample_from_json(row: &Value) -> Result<CapsuleSample> {
    ...
    Ok(CapsuleSample {
        ...
        dataset_repo_id: required_str(
            row,
            &["sample", "dataset_repo_id"],
            "sample.dataset_repo_id",
        )?
        .to_owned(),
        dataset_revision: required_str(
            row,
            &["sample", "dataset_revision"],
            "sample.dataset_revision",
        )?
        .to_owned(),
        audio_sha256: required_str(
            row,
            &["sample", "audio_sha256"],
            "sample.audio_sha256",
        )?
...
fn optional_assignment(benchmark: &Value) -> Result<Option<bool>> {
    let assigned = benchmark.pointer("/provider/assigned_nodes");
    match assigned {
...
fn required_u64(value: &Value, pointer: &str) -> Result<u64> {
    value
        ...
        .ok_or_else(|| invalid(format!("{pointer} must be an unsigned integer")))
}
...
fn optional_f64(value: &Value, pointer: &str) -> Result<Option<f64>> {
    match value.pointer(pointer) {
        ...
        Some(number) => {
            let value = number
                .as_f64()
                .ok_or_else(|| invalid(format!("{pointer} must be numeric or null")))?;
            if !value.is_finite() {
                return Err(invalid(format!("{pointer} must not be NaN/Infinity")));
            }
            Ok(Some(value))
        }
    }
