    ...
    pub fn load(path: impl AsRef<Path>) -> Result<Self> {
        ...
        match value {
            ...
            serde_json::Value::Object(items) => {
                for (k, v) in items {
                    let id = k.parse::<i64>().map_err(|_| {
                        EvalError::InvalidInput(format!("invalid vocabulary id {k}"))
                    })?;
                    map.insert(
                        id,
                        v.as_str()
            ...
            _ => {
                return Err(EvalError::InvalidInput(
                    "vocabulary JSON must be a list or object".into(),
                ));
            }
        }
    ...
    pub fn decode(&self, ids: &[i64]) -> Result<String> {
        ...
        for id in ids {
            out.push_str(
                self.id_to_token
                    .get(id)
                    .ok_or(EvalError::UnknownToken(*id))?,
            );
        }
        Ok(out)
    }
...
pub fn greedy_ctc_ids(logits: &[f32], shape: &[usize], blank_id: i64) -> Result<Vec<i64>> {
    ...
    if vocab == 0 || logits.len() != time * vocab {
        return Err(EvalError::InvalidInput(
            "logits shape/data length mismatch".into(),
        ));
    }
