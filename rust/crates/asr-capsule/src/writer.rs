...
use crate::{
    hash::sha256_file,
    model::{ONNX_CAPSULE_SCHEMA_VERSION, OnnxCapsule, OnnxCapsuleManifest, OnnxCapsuleMetric, OnnxCapsuleReceipt, OnnxCapsuleSample},
...
pub fn write_onnx_capsule(path: impl AsRef<Path>, capsule: &OnnxCapsule) -> Result<OnnxCapsuleReceipt> {
    capsule.validate()?;
    let path = path.as_ref();
    let file = File::create(path)
        .with_context(|| format!("failed to create ONNX capsule {}", path.display()))?;
    let schema = onnx_capsule_v1_schema();
    ...
    writer.write(&batch_from_rows(schema.clone(), &capsule.manifest, &[Row::manifest()])?)?;
    ...
    Ok(OnnxCapsuleReceipt {
        schema_version: ONNX_CAPSULE_SCHEMA_VERSION.to_owned(),
...
fn flush_if_needed(writer: &mut ArrowWriter<File>) -> Result<()> {
    if writer.in_progress_size() >= FLUSH_MEMORY_BYTES { writer.flush()?; }
    Ok(())
}
...
struct Row<'a> {
    ...
    ordinal: i64,
    sample: Option<&'a OnnxCapsuleSample>,
    metric: Option<&'a OnnxCapsuleMetric>,
}

impl<'a> Row<'a> {
    fn manifest() -> Self { Self { record_kind: "manifest", ordinal: 0, sample: None, metric: None } }
    fn sample(ordinal: i64, sample: &'a OnnxCapsuleSample) -> Self { Self { record_kind: "sample", ordinal, sample: Some(sample), metric: None } }
    fn metric(ordinal: i64, metric: &'a OnnxCapsuleMetric) -> Self { Self { record_kind: "metric", ordinal, sample: None, metric: Some(metric) } }
}

fn batch_from_rows(schema: Arc<Schema>, manifest: &OnnxCapsuleManifest, rows: &[Row<'_>]) -> Result<RecordBatch> {
    let n = rows.len();
    ...
    let columns: Vec<ArrayRef> = vec![
        Arc::new(repeated(ONNX_CAPSULE_SCHEMA_VERSION)),
        ...
        Arc::new(repeated(&manifest.runtime_backend)),
