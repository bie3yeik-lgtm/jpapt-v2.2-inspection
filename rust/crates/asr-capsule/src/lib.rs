...
pub use model::{
    CAPSULE_SCHEMA_VERSION, CapsuleManifest, CapsuleMetric, CapsuleReceipt, CapsuleSample,
    ExperimentCapsule,
};
pub use reader::read_capsule;
pub use schema::experiment_capsule_v1_schema;
pub use writer::write_capsule;
...
mod tests {
    ...
    fn fixture() -> ExperimentCapsule {
        ExperimentCapsule {
            ...
            samples: vec![CapsuleSample {
                sample_id: "s1".into(),
                dataset_id: "dataset".into(),
                dataset_repo_id: "org/dataset".into(),
                ...
                hypothesis_text: "hello".into(),
                normalized_text: "hello".into(),
                cer: Some(0.0),
                wer: Some(0.0),
                audio_decode_ms: Some(1.0),
                resample_ms: Some(0.0),
                inference_ms: Some(10.0),
                decoder_ms: Some(1.0),
                postprocess_ms: Some(0.5),
                total_ms: 12.5,
                rtf: Some(0.0125),
                peak_ram_mb: Some(128.0),
    ...
    #[test]
    fn capsule_round_trip_preserves_semantics() {
        let path = env::temp_dir().join(format!("asr-capsule-{}.parquet", std::process::id()));
        let expected = fixture();
        let receipt = write_capsule(&path, &expected).unwrap();
        assert_eq!(receipt.sample_count, 1);
        assert_eq!(receipt.metric_count, 1);
        assert_eq!(receipt.sha256.len(), 64);
        let observed = read_capsule(&path).unwrap();
