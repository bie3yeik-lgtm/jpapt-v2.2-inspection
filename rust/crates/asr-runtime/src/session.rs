...
use crate::{
    metadata::model_metadata::{InputKind, RuntimeContract},
    providers::{self, ProviderKind},
    tensors::LogitsTensor,
    Result, RuntimeError,
};
...
impl SessionConfig {
    pub fn new(model_path: impl AsRef<Path>, provider: ProviderKind) -> Self {
        Self {
...
impl OrtCtcSession {
    pub fn create(config: SessionConfig, contract: RuntimeContract) -> Result<Self> {
        if contract.input_kind != InputKind::CanonicalWaveform {
            return Err(RuntimeError::UnsupportedContract(
                "Rust runtime currently requires canonical_waveform input; external NeMo features stay in the Python export/reference layer".into(),
            ));
        }

        let builder = Session::builder()?;
        let mut builder = providers::configure(builder, config.provider)?;
        let session = builder.commit_from_file(&config.model_path)?;

        let names: Vec<&str> = session.inputs().iter().map(|input| input.name()).collect();
        if !names.iter().any(|name| *name == contract.primary_input) {
            return Err(RuntimeError::InvalidMetadata(format!(
                "primary input '{}' not found in model inputs {names:?}",
                contract.primary_input
            )));
        }
        if let Some(length) = &contract.length_input {
            if !names.iter().any(|name| *name == length) {
                return Err(RuntimeError::InvalidMetadata(format!(
                    "length input '{length}' not found in model"
                )));
            }
        }
        if !session
    ...
    pub fn contract(&self) -> &RuntimeContract {
        &self.contract
    }
    ...
    pub fn run_waveform(&mut self, samples: &[f32]) -> Result<InferenceOutput> {
        ...
        let outputs = if self.contract.length_input.is_some() {
            let length = Tensor::from_array((
                [1usize],
                vec![samples.len() as i64].into_boxed_slice(),
            ))?;
        ...
        let shape = shape.iter().map(|&dimension| dimension.max(0) as usize).collect();
